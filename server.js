...
app.get('/api/exhibits', async (req, res) => {
    try {
        ...
        const sql = 'SELECT id, "영업팀", sr_name, "매장명", "제품_모델명", "구분", "타입", "최초전시일", "구성품", "색상", "제품상태", "제품이상_메모", "정상가격", images, created_at FROM exhibit_data ORDER BY created_at DESC';
...
app.post('/api/exhibits', async (req, res) => {
    ...
    const 제품_모델명 = 제품_모델명_from_req;


    const imagesStr = JSON.stringify(images || []); 

    const sql = `INSERT INTO exhibit_data
        ("영업팀", "sr_name", "매장명", "제품_모델명", "구분", "타입", "최초전시일", "구성품", "색상", "제품상태", "제품이상_메모", "정상가격", "images")
